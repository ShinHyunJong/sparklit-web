...
function PhotoController() {
  ...
  const onCropComplete = async (croppedArea, croppedAreaPixels, imageId) => {
    const targetImage = staticUploadingImageList.find((x) => x.id === imageId);
  ...
  const handleCrop = (c: CropPos, imageId: number) => {
    if (!selectedImage) return;
    const prev = [...uploadingImageList];
    const targetIndex = prev.findIndex((x) => x.id === imageId);
    const target = prev[targetIndex];
